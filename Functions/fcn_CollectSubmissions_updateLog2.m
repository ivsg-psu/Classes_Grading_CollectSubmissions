function fcn_CollectSubmissions_updateLog2(logFile, ...
	syncTime, processDuration, flagWasSuccessful, subFolder, ...
	totalsCollected, finalGradedRosterTable, varargin)

%% fcn_CollectSubmissions_updateLog2
%     fcn_CollectSubmissions_updateLog2 updates a log file so that changes
%     in algorithm behavior can be tracked.
%
% FORMAT:
%
%      fcn_CollectSubmissions_updateLog2(logFile, ...
%      syncTime, processDuration, flagWasSuccessful, totalsCollected, ...
%      finalGradedRosterTable, (figNum));
%
%
% INPUTS:
%
...
%
%      finalGradedRosterTable: a table representing the class roster, with a
...
% EXAMPLES:
%
%     See the script: script_test_fcn_CollectSubmissions_updateLog2
...
if flag_do_plots
	...
	Nstudents = height(finalGradedRosterTable);
	...
	if ~hasPlot
		...
		ylim([-1 Nstudents]);
		ylabel('Number of students','Interpreter','none');
		xlabel('Timestamp');
		grid on;
