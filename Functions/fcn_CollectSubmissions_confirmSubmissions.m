...
%      verification. Default is the word 'SUBMISSION'
%
...
if ~ismember('preferencesSendConfirmationEmailOnSubmission',updatedRosterTable.Properties.VariableNames)
	notificationPreferences = true(height(updatedRosterTable),1);
else
	notificationPreferences = updatedRosterTable.('preferencesSendConfirmationEmailOnSubmission');
end

% Check to see if email flag column already exists. If not, create it
emailVerificationColumnName = 'confirmationEmailSent';
...
for ith_change = 1:length(fileContent)
	...
	if ~isempty(thisChangeChar)
		if length(thisChangeChar)>length(assignmentString) && contains(thisChangeChar,assignmentString)
			...
			if contains(thisChangeChar(1),'+')  % Addition
				...
				% Get student details
				studentEmail = updatedRosterTable.('PSUEmail'){studentRowNumber};
				...
				% Send email?
				thisStudentReceivedConfirmation = updatedRosterTable.(emailVerificationColumnName)(studentRowNumber);
