% script_test_fcn_CollectSubmissions_loopOverCollections.m
% tests fcn_CollectSubmissions_loopOverCollections.m
...
dataFiles = dir(fullfile(rostersFolder,'roster*.mat'));

if isempty(dataFiles)
	fprintf(1,'Loading the class roster using Canvas.\n');
	...
	mostRecent = dataFiles(end).name;
	fprintf(1,'Loading the most recent class roster: %s.\n', mostRecent);
	load(fullfile(rostersFolder,mostRecent),'rosterTable');
end

%%%%%%
%  Set variables and core paths
pathToMirrorFolder = fullfile(pwd,'CloudMirror');
pathToArchiveFolder = fullfile(pwd,'Archive');
...
localFolder = pathToMirrorFolder;
archiveFolder = pathToArchiveFolder;
